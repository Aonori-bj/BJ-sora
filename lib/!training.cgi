...
sub tp_1 {
	if ($cmd eq '1') {
		...
		while (my $line = <$fh>) {
			my($no,$name,$country,$max_hp,$max_mp,$at,$df,$mat,$mdf,$ag,$cha,$wea,$skills,$mes_win,$mes_lose,$icon) = split /<>/, $line;
			...
			$mes .= qq|<input type="radio" name="cmd" value="$no"><font color="$font_color"> $name ${e2j{hp}}[$max_hp] 強さ[$y_st] 属性[$weas[$wea][2]]</font><br>|;
		}
		seek  $fh, 0, 0;
		truncate $fh, 0;
		close $fh;
...
sub tp_100 {
	if ($cmd eq 'myself') {
		for my $k (qw/country max_hp max_mp at df mat mdf ag cha wea skills mes_win mes_lose icon/) {
			$y{$k} = $m{$k};
		}
		$y{name} = 'ｼｬﾄﾞｳ';
		$y{hp}   = $m{max_hp};
		...
		while (my $line = <$fh>) {
			...
			if ($name eq $m{name}) {
				$is_myself = 1;
				$line = "$no<>$m{name}<>$m{country}<>$m{max_hp}<>$m{max_mp}<>$m{at}<>$m{df}<>$m{mat}<>$m{mdf}<>$m{ag}<>$m{cha}<>$m{wea}<>$m{skills}<>$m{mes_win}<>$m{mes_lose}<>$m{icon}<>\n";
			}
			push @lines, $line;
		...
		unless ($is_myself) {
			...
			++$last_no;
			unshift @lines, "$last_no<>$m{name}<>$m{country}<>$m{max_hp}<>$m{max_mp}<>$m{at}<>$m{df}<>$m{mat}<>$m{mdf}<>$m{ag}<>$m{cha}<>$m{wea}<>$m{skills}<>$m{mes_win}<>$m{mes_lose}<>$m{icon}<>\n";
		}
		seek  $fh, 0, 0;
		truncate $fh, 0;
		print $fh @lines;
