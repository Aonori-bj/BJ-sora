...
sub begin {
	...
	&menu('‚â‚ß‚é','¤•i‰{——', '“X“ª‚É’u‚­', '‚¨“X‚ÌĞ‰î', '‚¨“X‚ğŒš‚Ä‚é');
}
...
sub tp_400 {
	if ($cmd eq '1') {
		if (-f $this_file) {
			...
			print $fh3 "$m{name}“X<>$m{name}<>$dateŠJ“X<>0<>0<>\n";
			close $fh3;
...
sub tp_100 {
	...
	$mes .= qq|<p><input type="submit" value="—a‚©‚èŠ‚É–ß‚·" class="button1"></p></form>|;
...
sub tp_200 {
	...
	while (my $line = <$fh>) {
		...
		$mes .= $kind eq '1' ? qq|<input type="radio" name="cmd" value="$i">[$weas[$item_no][2]]$weas[$item_no][1]š$item_lv($item_c/$weas[$item_no][4])<br>|
			  : $kind eq '2' ? qq|<input type="radio" name="cmd" value="$i">[—‘]$eggs[$item_no][1]($item_c/$eggs[$item_no][2])<br>|
			  :				   qq|<input type="radio" name="cmd" value="$i">[‚Ø]$pets[$item_no][1]<br>|
			  ;
		++$i;
	}
	close $fh;
...
sub tp_210 {
	...
	if ($cmd) {
		...
		if (@shop_items >= $max_shop_item) {
			...
			$mes .= '’l’i‚Í 1 G ˆÈã 499–œ9999 GˆÈ“à‚É‚·‚é•K—v‚ª‚ ‚è‚Ü‚·<br>';
			&begin;
			return;
		}
...
sub tp_310 {
	...
	&error('Ğ‰î•¶‚ª’·‚·‚¬‚Ü‚·B‘SŠp20(”¼Šp40)•¶š‚Ü‚Å') if length $in{mes} > 40;
