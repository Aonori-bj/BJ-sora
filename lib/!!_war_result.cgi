...
sub war_win {
	...
	if ($m{pet} eq '193' && !($w{world} eq '17' || ($w{world} eq '19' && $w{world_sub} eq '17')) ) { # 白兵ではないﾀﾞｰﾗﾋﾞ時、階級補正と世代補正に進軍補正のみ
		...
		# 国情勢により奪国力増加
		if (($w{world} eq '5' || $w{world} eq '6' || ($w{world} eq '19' && ($w{world_sub} eq '4' || $w{world_sub} eq '5')))) { # 暴君、混沌
			$v *= 2.5;
		}
...
#						$v = $v > 600  ? int(rand(200)+400) : int($v);
	...
	if ($w{world} eq $#world_states - 5) {
		...
		&write_world_news(qq|$c_mの$m{name}が<font color="#FF00FF"><b>$v</b> の$e2j{strong}を得る事に成功</font>したようです|);
	} else {
...
=pod#system.cgiのname_link関数ができたら戻す　by　あおのり
	if ($w{world} eq $#world_states - 5) {
		$mes .= "$vの$e2j{strong}を得ました<br>";
...
#			$mes .= "<br>ｶｳﾝﾀｰ発生しないので確認用 $dividend / $divisor = $div<br>";
	...
	# 鉄壁
	elsif (($w{world} eq '8' || ($w{world} eq '19' && $w{world_sub} eq '7')) && $cs{strong}[$y{country}] <= 3000 && rand(3) < 1) {
