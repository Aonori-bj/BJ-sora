...
=cut
	...
	if ($cs{strong}[$w{country}] < 30000) {
		...
		:                &npc_get_strong
		;
	}#ﾌｪﾝﾘﾙ1/6 → 1/8 → 1/6（減少幅が小さすぎた）
	elsif ($cs{strong}[$w{country}] < 50000) {
		  rand(5)  < 1 ? &npc_use_pet_fenrir # ｶｳﾝﾀｰ率が統一国力に左右されるので、無改造よりもﾌｪﾝﾘﾙ発動率上げてｶｳﾝﾀｰ率変動させたい
...
#	my $holy_mem = int($w{country} * 0.5) * 3;
	my $vv = int(6 - $cs{member}[$w{country}]) * 40; #暗黒の人数に合わせて奪国調整
	if ($vv) {
		$vv = $vv > 240 ? 240 :
				$vv < -240 ? -240 :
				$vv; # 最高+-300
	...
	if($cs{strong}[$w{country}] < 50000 && $cs{strong}[$w{country}] > 30000){
		my $vv = -20;#中盤調整
		$v += int($vv);
	}
	if($cs{strong}[$w{country}] < 30000){
		my $vv = -50;#終盤になると反撃率の増加で強くなりすぎる
		#本来は上のコードのように、ペット発生確率と同様の確率処理を入れるべきだが
		#下手にいじると面倒なので、奪国調整と言う形で調整を入れた　あおのり
		$v += int($vv);
	}
