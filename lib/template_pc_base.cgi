...
sub status_html {
	...
	if (-f "$userdir/$id/letter_flag.cgi") {
		$head_mes .= qq|<font color="#FFCC66">è†‚ª“Í‚¢‚Ä‚¢‚Ü‚·</font><br>|;
	...
	if ($m{country}) {
		...
		$country_info .= qq|<hr size="1">|;
		$country_info .= qq|Ÿ‚Ì‹‹—^‚Ü‚Å <span id="nokori_time">$nokori_time_mes</span>\n|;
	...
	$main_screen .=<<"EOM";
		...
		<font color="#9999CC">•ŠíF[$weas[$m{wea}][2]]$weas[$m{wea}][1]š<b>$m{wea_lv}</b>(<b>$m{wea_c}</b>/<b>$weas[$m{wea}][4]</b>)</font><br>
		<font color="#99CCCC">Íß¯ÄF$pets[$m{pet}][1]</font><br>
...
sub battle_html {
	...
	$main_screen .= qq|<tr><td colspan="3">$m_tokkou•ŠíF[$weas[$m{wea}][2]] $weas[$m{wea}][1]š$m{wea_lv} ($m{wea_c})<br></td></tr>| if $m{wea};
	$main_screen .= qq|<tr><td colspan="3">Íß¯ÄF$pets[$m{pet}][1]<br></td></tr>| if $pets[$m{pet}][2] eq 'battle';
	...
	$main_screen .= qq|<tr><td colspan="3">$y_tokkou•ŠíF[$weas[$y{wea}][2]] $weas[$y{wea}][1]<br></td></tr>| if $y{wea};
...
sub promise_table_html {
	...
	print qq|<table class="table1"><tr><td>ó‘Ô/—FD“x</td>|;
