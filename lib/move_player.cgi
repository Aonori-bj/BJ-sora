...
sub move_player {
	...
	if ($to_country eq 'del') {
		...
		mkdir "${userdir}_trash/$p_id" or &error("ｺﾞﾐ箱に同名ﾌﾟﾚｲﾔｰのﾌｧｲﾙが存在します");
		...
		while (my $file_name = readdir $dh) {
			...
			if (-d "$userdir/$p_id/$file_name") {
				unless (-d "${userdir}_trash/$p_id/$file_name") {
					mkdir "${userdir}_trash/$p_id/$file_name" or &error("ｺﾞﾐ箱のﾃﾞｨﾚｸﾄﾘ作成に失敗しました");
				}
				opendir my $dh2, "$userdir/$p_id/$file_name";
				while (my $file_name2 = readdir $dh2) {
					next if $file_name2 =~ /^\./;
					rename "$userdir/$p_id/$file_name/$file_name2", "${userdir}_trash/$p_id/$file_name/$file_name2" or &error("ﾕｰｻﾞｰﾃﾞｰﾀの移動に失敗しました");
				}
				closedir $dh2;
				...
				rename "$userdir/$p_id/$file_name", "${userdir}_trash/$p_id/$file_name" or &error("ﾕｰｻﾞｰﾃﾞｰﾀの移動に失敗しました");
			}
		}
		closedir $dh;
		rmdir "$userdir/$p_id";
		...
		if ($datas{icon} ne $default_icon && -f "$icondir/$datas{icon}") {
			rename "$icondir/$datas{icon}", "${userdir}_trash/$p_id/picture/$datas{icon}" or &error("ｱｲｺﾝﾃﾞｰﾀの移動に失敗しました");
		}
