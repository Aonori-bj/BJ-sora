...
sub tp_400 {
	...
	while (my $line = <$fh>) {
		...
		$mes .= $price > 999999 ? qq|<td align="right">非売品($price G)<br></td></tr>| : qq|<td align="right">$price G<br></td></tr>|;
	}
	close $fh;
...
sub tp_500 {
	...
	$mes .= "値段を100万G以上にすることで非売品にすることができます<br>";
	$mes .= qq|<form method="$method" action="$script">|;
	$mes .= &radio_my_goods;
...
sub tp_510 {
	...
	if (!$in{file_name}) {
		...
		$mes .= $in{price} > 999999 ? "$file_titleを非売品として展示しました<br>" : "$file_titleを $in{price} Gで展示しました<br>";
		&tp_500;
	}
	else {
		&begin;
