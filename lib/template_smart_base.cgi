...
sub check_flag {
	...
	if (-f "$userdir/$id/letter_flag.cgi") {
		#$letters‚Ìˆ—‚ğ’Ç‚¦‚È‚©‚Á‚½‚Ì‚Å‰‹}ˆ’u‚ğ‚µ‚½
...
sub my_country_info {
	...
	print qq|<div class="c_info1c">é•Ç:$cs{barrier}[$m{country}]%</div>|;
...
#		print qq|<br>|;
		...
		print qq|<div class="c_info1c">é•Ç:$cs{barrier}[$union]%</div>|;
...
sub all_member_n {
	...
	if (($lastmodified + 180) < $time) { # 3•ª–ˆ‚É‘Îl¶¼ŞÉ‚Ìl”XV
		for my $i (0 .. $#files) {
			my $member_c  = 0;
			my %sames = ();
		...
		open my $fh, "> $casino_n_file" or &error('‘Îl¶¼ŞÉ‚Ìl”Ì§²Ù‚ªŠJ‚¯‚Ü‚¹‚ñ');
		print $fh $ret_str2;
		close $fh;
	}
	else {
