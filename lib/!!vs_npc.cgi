...
sub npc_use_pet_fenrir { # ﾌｪﾝﾘﾙ
	...
	my $A = rand(400) + 400;
	my $B = rand(200) + 200;
#	$strong_eq = $strong_eq *(2 - $w{player}:0.01);
#	$strong_eq = 0.5 if $strong_eq < 0.5;
	for my $i (1..$w{country}-1) {
		next if $cs{is_die}[$i];
		next if $cs{strong}[$i] < 1000;
		$cs{strong}[$i] -= $touitu_strong * 0.6 > $cs{strong}[$w{country}] ? int($A*$strong_eq) : int($B*$strong_eq);
		$cs{barrier}[$i] -= 5;
		$cs{barrier}[$i] = 0 if $cs{barrier}[$i] < 0;
	}
	$cs{barrier}[$w{country}] += 5;
	$cs{barrier}[$w{country}] = 100 if $cs{barrier}[$w{country}] > 100;
#	$cs{strong}[$w{country}] -= $touitu_strong * 0.6 > $cs{strong}[$w{country}] ? int($A*(1 - $strong_eq)) : int($B*(1 - $strong_eq));
...
　#上の奪国補正をいじって、（なぜか）導入されてない暗黒の人数補正を追加する
	my $all_mem = 1; #100